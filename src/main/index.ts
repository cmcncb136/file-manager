...
import { ImageMappingService } from '../application/imageMappingService'
...
function createWindow(): void {
  ...
  ipcMain.handle('application-call', async (_, { service, method, payload }) => {
    ...
    console.log('s ' + service + ' method : ' + method)
    console.log('type : ' + s[method])
    if (typeof s[method] !== 'function') throw new Error(`Method ${method} not found`)
    if (payload) return await s[method](...payload)
    else return await s[method](payload) //payload가 null인 경우
  })
}
